...
public class Main extends Activity{
	...
	{        
		...
		ArrayAdapter<CharSequence> stateAdapter = ArrayAdapter.createFromResource(customListPopup.getContext(), R.array.State, android.R.layout.simple_spinner_item);
		stateAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		stateSpinner.setAdapter(stateAdapter);

		Spinner importanceSpinner = (Spinner) customListPopup.findViewById(R.id.importanceSpinner);
		ArrayAdapter<CharSequence> priorityAdapter = ArrayAdapter.createFromResource(customListPopup.getContext(), R.array.Importance, android.R.layout.simple_spinner_item);
	...
	private void prepareListData() {
		...
		for (int i = 0; i < beans.size() && i < 3; i++) {
			CustomList.add(0,beans.get(i).getListName());
		}
	...
	protected Dialog onCreateDialog(int id){
		...
		case taskEdit.DATE_DIALOG:
			...
			((AlertDialog) dialog).setButton(DialogInterface.BUTTON_NEGATIVE, "Clear",
				...
				@Override
				public void onClick(DialogInterface dialog, int which) {
					if (statistics.dOrs == 0){
						statistics.eDate.setText("");
					}else if (statistics.dOrs == 1){
						statistics.sDate.setText("");
						statistics.dOrs = -1;
					}else if (statistics.dOrs == 2) {
						csDate.setText("");
					}else if (statistics.dOrs == 3) {
